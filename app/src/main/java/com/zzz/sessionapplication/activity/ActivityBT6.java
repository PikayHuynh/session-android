...
public class ActivityBT6 extends AppCompatActivity {
    ...
    private void act() {
        ...
        BiFunction<String, Boolean, String> message = (value, isChecked) ->
                isChecked ?
                        String.format("Bạn chọn '%s'.", value)
                        :
                        String.format("Bạn không chọn '%s'.", value);

        checkBoxPuzzle.setOnCheckedChangeListener((v, isChecked) ->
                Toast.makeText(this, message.apply(PUZZLE, isChecked), Toast.LENGTH_SHORT).show());
        checkBoxSpell.setOnCheckedChangeListener((v, isChecked) ->
        ...
        btnSubmit.setOnClickListener(v -> {
            ...
            String result =
                    message.apply(PUZZLE, checkBoxPuzzle.isChecked()) + ", " + message.apply(SPELL, checkBoxSpell.isChecked());
